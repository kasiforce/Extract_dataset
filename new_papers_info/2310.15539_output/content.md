Preprint
STELOCODER: A DECODER-ONLY LLM FOR MULTI-
...
1
arXiv:2310.15539v2  [cs.CL]  15 Dec 2023
...
Since the introduction of the transformer and
...
Pre-Training for Programming Language Generation
More recently, the scientific community
...
Since the release of LLaMA (Touvron et al., 2023), the open
...
Gururangan et al. (2021) introduces an expert mixture in the transformer
...
Assuming that the parameters update has an intrinsic rank r much lower than its actual rank, we can
rewrite it as ∆W = B × A where ∆W ∈Rd×k, A ∈Rr×k and B ∈Rd×r. A and B are the only
trainable parameters, while W remains frozen. ∆W is then scaled by α
r , where α is a constant, and r
...
Figure 1: In inference, LoRA weights A and B can either be seamlessly merged by updating W with
W ′ = W + α
...
LoRA factor is set to r = 4 (and α = 32). This sums up to an additional 9M parameters per expert.
...
DATA PRE-PROCESSING
JSON Format
The data samples, organized in pairs, are initially stored in two separate files, with
...
Self-Instruct Data
To prompt the model with a supervised format (Wang et al., 2023), we introduce
...
Data Samples construction
To create the final dataset, we concatenate the two key-value pairs
...
We employ the LoRA method for fine-tuning StarCoder. This
...
performance.
Fine-tuning with CL Strategy
As previously mentioned, we adopt a CL strategy to fine-tune the
...
CodeBLEU (single expert)
CodeBLEU (1st in XLCoST)
cpp-to-python
76.62
71.56
csharp-to-python
...
matrix W and is also simultaneously multiplied with LoRA matrices A and then B. A computationally
efficient alternative is to introduce the new matrix C = B × A, computed only once. Whenever an
input is going through a LoRA-decomposed matrix, one can first merge C in W and perform the
matrix multiplication. Afterwards, the C matrix is unmerged from W ′ using a simple subtraction.
...
Tom Kocmi, , and Ondˇr ej Bojar. Curriculum learning and minibatch bucketing in neural machine
...
URL https://doi.org/10.26615%2F978-954-452-049-6_050.
...
s t d ;
bool
i s P e r f e c t S q u a r e ( long
double x ) {
long
double
s r = s q r t ( x ) ;
...
i f
( i s P e r f e c t S q u a r e (N + 1 ) )
{
cout << " Yes STRNEWLINE" ;
}
e l s e
...
f l o o r
def
i s P e r f e c t S q u a r e ( x ) :
s r = s q r t ( x )
return
( ( s r −f l o o r ( s r ) )
== 0)
...
checkSunnyNumber (N) :
i f
( i s P e r f e c t S q u a r e (N +
1 ) ) :
print ( " Yes " )
e l s e :
print ( "No" )
...
re2 = b / c
i f
( ( re1 * re1 * re2 == a ) and ( re2 * re2 * re1 == b ) ) :
return 1
e l s e :
return −1
A = 60
...
i n t
n = a r r . l e n g t h ;
System . out . p r i n t l n ( ways ( arr ,
n ) % mod ) ;
}
}
mod = 1000000007
