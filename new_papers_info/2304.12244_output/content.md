...
ChatGPT 1) have achieved great success. These open-domain instructions can fully unleash the
...
1
arXiv:2304.12244v3  [cs.CL]  27 May 2025
...
How is the speed of light in a 
vacuum measured and defined?
...
(instructions are created by human). Alpaca data has a total of 52k samples and is generated using
self-instruct (Wang et al., 2022a) from only 175 human-created seed instructions. We choose Alpaca
...
Published as a conference paper at ICLR 2024
using OpenAI ChatGPT API2 and finally obtain 250k instructions. To ensure a fair comparison with
Vicuna’s 70k real user data, we sampled 70k from the full 250k data and fine-tuned the LLaMA 13B
model. Because the original Alpaca data only has 52k samples, we used its self-instruct method to
generate an additional 18k data, and retrained the LLaMA 13B model with its code3 to get Alpaca 13B
...
Our work belongs to this research line. OpenAI has hired
...
number of samples in D(0). In each evolution, we upgrade all the I(t) in D(t) to I(t+1) by prompting
...
performing M evolutions, we can sequentially obtain M evolution datasets [D(1) · · · D(M)]. Our
...
We found that LLMs can make given instructions more complex and
...
Instruction Evolver.
The Instruction Evolver is an LLM that uses Evol-Instruct prompts to evolve
...
#Created Prompt#:
Response Generation.
We use the same LLM as for evolving to generate the corresponding
...
fair comparison, we expanded the number of instructions from 52k to 70k using self-Instruct adopted
...
(3) Vicuna is based on LLaMA and fine-tuned on 70k user-shared conversations collected from
ShareGPT. It is one of the most advanced and versatile open instruction-following models available
today. We use the 13B-v1.1 model from FastChat 4.
...
4.2
EXPERIMENT DETAIL
To construct the dataset, we initialize it with the 52k instruction dataset of Alpaca and iteratively
perform M evolutions, where M = 4. For each instruction in each round of evolution, we randomly
...
API5. Then, we leverage ChatGPT to generate responses. Finally, we obtain 250k instructions.
For a fair comparison, we randomly sample 70k data from 250k data with equal probability as the
...
× 4 × 3 = 624k times to construct the full dataset. We use pre-trained LLaMA 13B (Touvron et al.,
2023) to initialize our model. We adopt Adam optimizer with an initial learning rate of 2 × 10−5, a
maximum number of tokens 2048, and the batch size is 4 for each GPU. We train our model on 8
V100 GPUs with Deepspeed Zero-3 for 140 hours on 3 epochs. For inference, we use greedy search
for WizardLM and baseline models, and set the maximum generation length to 2048.
4.3
AUTOMATIC EVALUATION
Figure 3: Automatic evaluations on nine LLM
benchmarks.
To present a comprehensive overview of the perfor-
...
6.51
90.3
WizardLM-13b (LlaMA-2-70B-Chat Evol)
56.27
51.09
57.34
79.12
...
fine-tuned with the data from each evolution round. Each round of data from C0 to C4 is about
...
I
DIFFERENT DIFFICULTY ANNOTATORS
