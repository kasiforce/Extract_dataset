...
due to the use of unintuitive variable names [20]. Second,
...
publicly available source code to make them better suited
...
Tufano et al.’s ATHENATEST [26] generates tests using a
...
techniques in a system called TESTPILOT, an LLM-based
...
their quality in terms of success rate, reasons for failure,
and whether or not they contain assertions that actually
...
as whether TESTPILOT is generating previously memorized
tests from the LLM’s training data.
Using OpenAI’s current most capable and cost-effective
model gpt3.5-turbo,1 TESTPILOT’s generated tests achieve a
...
3
let pkg = require('package-under-test');
4
5
// function metadata, including signature of f
6
7
describe('test pkg', function() {
8
it('test f', function(done) {
9
// test code, terminated by done()
10
})
11
})
...
tool called TESTPILOT, which is available as open-source
software at https://github.com/githubnext/testpilot.
• An extensive empirical evaluation of TESTPILOT on 25
...
passed to the it function. Test suites consist of one or more
calls to it that occur in a callback function that is passed
...
accepting a parameter done passed to the it function. The
...
in blue) as a prompt to the LLM. Since LLMs are trained
to complete a given code fragment, one might therefore
...
Figure 2 presents the high-level architecture of TESTPILOT,
...
7
let country = countries_and_timezones.getCountry('US');
8
assert.equal(country.name, 'United States'); // fails
9
assert.equal(country.timezones.length, 2);
10
assert.equal(country.timezones[0], 'America/New_York');
11
assert.equal(country.timezones[1], 'America/Chicago');
12
done();
13
})
...
it('test countries-and-timezones.getCountry', function(done) {
23
let country = countries_and_timezones.getCountry('DE');
24
assert.equal(country.id, 'DE');
25
assert.equal(country.name, 'Germany');
26
assert.equal(country.timezones[0], 'Europe/Berlin');
27
assert.equal(country.timezones[1], 'Europe/Zurich');
28
done();
29
})
...
have (at most) four pieces of information about f at our
...
step by step with additional information.
Test Validator: Next, we send the generated prompts
...
2:
modObj ←object created by importing pkgName
3:
seen ←∅
4:
return explore(pkgName, modObj, seen)
...
seen ←seen ∪{ obj }
9:
if obj is a function with signature sig then
10:
apis ←apis ∪{ ⟨accessPath, sig⟩}
11:
else if obj is an object then
12:
props ←{ prop | obj has a property prop }
13:
for prop in props do
14:
apis ←apis ∪
15:
explore(extend(accessPath, prop),obj[prop], seen)
16:
else if obj is an array then
17:
for each index i in the array do
18:
apis ←apis ∪
19:
explore(extend(accessPath, prop),obj[i], seen)
20:
return apis
21: function extend(accessPath, component)
22:
if component is numeric then
23:
...
refiners as follows:
1) FnBodyIncluder: If p did not contain the definition of f, a
prompt is created that includes it.
2) DocCommentIncluder: If f has a doc comment but p did
not include it, a prompt with the doc comment is created.
3) SnippetIncluder: If usage snippets for f are available but p
did not include them, a prompt with snippets is created.
4) RetryWithError: If t failed with error message e, a prompt
is constructed that consists of: the text of the failing test
t followed by a comment // the test above fails with
the following error: e, followed by a comment // fixed
test. This strategy is only applied once per prompt, so it
is not attempted if p itself was already generated by this
strategy.
...
⟨a, sig⟩representing its API. Here, a is an access path that
uniquely represents an API method, and sig is the signature
...
pair ⟨a, sig⟩is recorded (line 10). If the type of p is an object,
then the objects referenced by its properties are recursively
explored (lines 15–15), and if the type of p is an array, then
...
to prompts, containing only the access path and signature,
...
seen ←∅
5:
apis ←exploreAPI(pkgName)
▷See Algorithm 1
6:
for api ∈apis do
7:
prompts ←prompts ∪{ createBasePrompt(api) }
▷create base prompts containing only the signature, see Figure 1
8:
9:
promptsWithFnBody ←∅
▷refine prompts by adding function body
10:
for prompt ∈prompts do
11:
body ∈findFnBody(prompt.api.accessPath, prompt.api.sig)
12:
promptsWithFnBody ←promptsWithFnBody ∪refine(prompt, body)
13:
prompts ←prompts ∪promptsWithFnBody
14:
15:
promptsWithExamples ←∅
▷refine prompts in cases where example snippets are available
16:
for prompt ∈prompts do
17:
snippets ←findExamples(prompt.api.accessPath, prompt.api.sig)
18:
if snippets ̸= ∅then
19:
promptsWithExamples ←promptsWithExamples ∪refine(prompt, snippets)
20:
prompts ←prompts ∪promptsWithExamples
21:
22:
promptsWithDocComments ←∅
▷refine prompts in cases where doc comments are available
23:
for prompt ∈prompts do
24:
docComment ←findDocComments(prompt.api.accessPath, prompt.api.sig)
25:
if docComment ̸= ∅then
26:
promptsWithDocComments ←promptsWithDocComments ∪refine(prompt, docComment)
27:
prompts ←prompts ∪promptsWithDocComments
28:
29:
while prompts ̸= ∅do
30:
select and remove prompt from prompts
31:
completions ←getCompletions(LLM, prompt.text)
▷request completions from the LLM
32:
for completion ∈completions do
33:
test ←concatenate(prompt, completion)
34:
test ←fixMinorSyntaxIssues(test)
▷e.g., add missing close parentheses
35:
test ←removeComments(test)
36:
if test is syntactically valid and test ̸∈seen then
37:
seen ←seen ∪{ test }
38:
...
▷add successful test to tests
40:
tests ←tests ∪{ test }
41:
else
▷result.status = assertionFailure or result.status = crash or result.status = nonTermination
42:
if prompt was not constructed from a previous failed test then
▷apply error retry refiner
43:
prompt′ ←refineFromError(test, result.errorMessage)
44:
...
two examples of how TESTPILOT generates tests.
As the first example, we consider the npm package
countries-and-timezones.2 API exploration reveals that this
package exports a function getCountry with a single pa-
rameter id and the project’s README.md file provides a usage
example.
...
assertion.
2. See https://www.npmjs.com/package/countries-and-timezones.
Next, we refine this prompt to include the usage snippet
...
for quill-delta’s concat method, which concatenates two
...
4
// quill-delta.prototype.concat(other)
5
...
{ insert: 'World!' }]);
11
let delta2 = new quill_delta([{ insert: 'Hello' },
12
{ insert: ' ',
13
attributes: { bold: true } },
...
{ insert: 'World!' }]);
15
let delta3 = delta1.concat(delta2);
16
assert.equal(delta3.ops.length, 6); // fails
17
done();
18
})
...
let delta3 = delta1.concat(delta2);
16
assert.equal(delta3.ops.length, 6);
17
done();
18
})
...
// fixed test:
23
it('test quill_delta', function(done) {
24
let delta1 = new quill_delta([{ insert: 'Hello' },
25
{ insert: ' ',
...
let delta3 = delta1.concat(delta2);
33
assert.equal(delta3.ops.length, 5);
34
done();
35
})
...
3
RESEARCH QUESTIONS & EVALUATION SETUP
3.1
Research Questions
Our evaluation aims to answer the following research ques-
tions.
RQ1 How much statement coverage and branch coverage do tests
...
We compare TESTPILOT’s coverage to the state-of-
...
proach and the effect of the underlying LLM TESTPI-
...
the model’s training set. For this reason, we also include
an additional 5 packages whose source code is hosted on
GitLab.4
...
To answer RQ1–RQ6, we run TESTPILOT using the
gpt3.5-turbo LLM (version gpt-3.5-turbo-0301), sampling five
completions of up to 100 tokens at temperature zero,5 with
...
9
TABLE 2: Statement and branch coverage for TESTPILOT’s passing tests, generated using gpt3.5-turbo. We also show passing
...
EVALUATION RESULTS
4.1
RQ1: TESTPILOT’s Coverage
Table 2 shows the number of tests TESTPILOT generates
...
of the tests generated by TESTPILOT are passing tests, with
...
The lowest statement coverage TESTPILOT achieves is on
...
on memfs (29.1% vs 29.3%), which may be due to platform differences.
repository.7 Including the extracted example snippets from
...
It is worth noting that TESTPILOT’s coverage for the
GitLab projects listed in the bottom 5 rows of Table 2 ranges
from 51.4% to 78.3%. This demonstrates that TESTPILOT is
...
in the code. While TESTPILOT does attempt to generate
...
proportion of the tests TESTPILOT generates for this package
...
of 77.1%. To ensure that TESTPILOT is not generating high
...
size, indicating that TESTPILOT is not only doing well for
smaller functions.8
...
two functions that TESTPILOT cannot cover, because their
corresponding generated tests fail either due to references to
non-existent files TESTPILOT includes in the tests or because
they time out. However, the functions that TESTPILOT is
...
how many of the tests TESTPILOT generates are uniquely
...
4.2
RQ2 TESTPILOT vs. Nessie
We compare TESTPILOT’s coverage to the state-of-the-art
...
Table 2, TESTPILOT’s median statement coverage is much
higher at 70.2%. The difference in branch coverage is even
higher, with 52.8% for TESTPILOT vs 25.6% for Nessie. Both
...
Nessie always generates 1000 tests per package, while TEST-
...
from the two techniques in bold. TESTPILOT outperforms
...
11
1
let manuelmhtr_countries_and_timezones = require("../../TEST_REPO_manuelmhtr_countries_and_timezones");
2
module.exports = function() {
3
let ret_val_manuelmhtr_countries_and_timezones_1;
4
try {
5
ret_val_manuelmhtr_countries_and_timezones_1 = manuelmhtr_countries_and_timezones.getAllCountries();
6
Promise.resolve(ret_val_manuelmhtr_countries_and_timezones_1).catch(e => { console.log({"error_1": true}); });
7
} catch(e) {
8
...
10
let ret_val_manuelmhtr_countries_and_timezones_2;
11
try {
12
ret_val_manuelmhtr_countries_and_timezones_2 =
13
manuelmhtr_countries_and_timezones.getCountry({"k": -293.76984807333383,
14
"rHMR": -17.71151399309167,
15
"vSF6": 721.0602634375625,
16
"l": 497.17371230897766,
17
"EnL": -611.9090030925536});
18
Promise.resolve(ret_val_manuelmhtr_countries_and_timezones_2).catch(e => { console.log({"error_2": true}); });
19
} catch(e) {
20
...
js), TESTPILOT achieves lower coverage than Nessie. For
...
Nessie and TESTPILOT, tests generated by Nessie tend
to look quite different from the ones generated by TEST-
...
contain at least one non-trivial assertion.
Table 3 shows the number of tests with non-trivial as-
...
one
package,
image-downloader where TESTPILOT generates only trivial
...
9.1% – 94.6% of TESTPILOT’s generated tests per package
...
4.4
RQ4: Characteristics of Failing Tests
...
Without Error Msgs
Without Example Snippets
Without Doc Comments
Base Prompt
Fig. 8: Effect of disabling prompt refiners in TESTPILOT,
...
tance between the two coordinates. However, TESTPILOT
...
this specific case, when TESTPILOT re-prompted the model
...
son) allows TESTPILOT to produce a consequent passing test
in 15.6% of the cases.
4.5
RQ5: Effect of Prompt Refiners
...
4
// complex.js.ZERO.valueOf()
5
describe('test suite', function() {
6
it('test case', function(done) {
7
assert.equal(complex_js.ZERO.valueOf(), 0);
8
done();
9
})
...
13
assert.equal(complex_js.ZERO.valueOf(), null);
14
done();
15
})
...
the Without Doc Comments configuration contains data for
only 8 packages.
...
the results (SnippetIncluder on crawler-url-parser and dirty;
and RetryWithError on gitlab-js and zip-a-folder), disabling
...
the refiners (e.g., FnBodyIncluder or SnippetIncluder) results
in a large drop in the percentage of passing tests. This
...
across all packages, a full configuration always leads to a
...
omnitool
all
Fig. 10: Cumulative percent of TESTPILOT generated test
...
coverage, and that TESTPILOT is applicable even in cases
...
part of its training set. This raises the concern that TESTPI-
...
isting tests as follows: maxtp∈TP

1 −
dist(t∗,tp)
max(len(t∗),len(tp))

,
where TP is the set of existing test functions in a package,
it('test case', function(done) {
...
(b)
Fig. 11: Example of a TESTPILOT-generated test case for
...
tive percentage for all generated test cases across all projects.
We find that 6.2% of TESTPILOT’s generated test cases have
less than ≤0.3% maximum similarity to an existing test,
60.0% have ≤0.4 similarity, 92.8% have ≤0.5, 99.6% have
≤0.6 while 100.0% of the generated tests cases have ≤0.7.
This means that TESTPILOT never generates exact copies of
existing tests. In contrast, while 90% of Lemieux et al. [27]’s
generated Python tests have ≤0.4 similarity, 2% of their
...
> 0.5 similarity do not pose a concern that TESTPILOT is
...
15
TABLE 4: A comparison of statement coverage of TESTPILOT’s generated tests using three LLMs. For each project, we show
...
by gpt3.5-turbo, with the latter having a slightly higher
...
that using gpt3.5-turbo with TESTPILOT resulted in higher
...
Finally, we note that the median time for TESTPILOT
to generate tests for a given function using gpt3.5-turbo is
15s, and the median time to generate a complete test suite
for a given package is 6m 55s.12 The bulk of this time is
spent querying the model, so the choice of LLM makes a
big difference. For example, the median time for TESTPILOT
...
use TESTPILOT either in an online setting (e.g., in an IDE)
...
for any usage of the package under test in the backwards
...
code bases. In particular, TESTPILOT’s performance may not
...
eration [23]–[29]. There are two main differences between
our work and these efforts: (i) the goal and types of tests
generated and (ii) the need for some form of fine-tuning or
additional data. We discuss the details below.
Differing goals:
TICODER [24] and CODET [23]
...
and a set of test cases to validate its correctness. CODET, on
...
target 27 Python projects, their proposed technique, CO-
DAMOSA, outperforms the baseline search-based technique,
Pynguin’s implementation of MOSA [64], as well as using
...
the model on a large set of test cases whereas TESTPILOT
...
analysis is performed to deduce relationships between pro-
gram elements such as variables and expressions. Then, a
...
6.2
Test Generation Techniques for JavaScript
...
By contrast, TESTPILOT only requires the functions of API
functions under test to be available and executable, and it
executes each test that it generates only once.
7
CONCLUSIONS AND FUTURE WORK
We have presented TESTPILOT, an approach for adaptive
unit-test generation using a large language model. Unlike
previous work in this area, TESTPILOT requires neither fine
tuning nor a parallel corpus of functions and tests. Instead,
...
of the tests generated by TESTPILOT. While in this paper
...
such as TESTPILOT. For example, one could use an LLM-
...
teresting to explore is how well an approach like TESTPILOT
...
Addison-Wesley, 2000.
[2]
J. Shore and S. Warden, The Art of Agile Development, 2nd ed.
O’Reilly, 2021.
[3]
S. Siddiqui, Learning Test-Driven Development.
O’Reilly, 2021.
[4]
E. Daka and G. Fraser, “A survey on unit testing practices and
...
[6]
M. Zalewski, “American fuzzy lop,” https://lcamtuf.coredump.
cx/afl/, 2022.
[7]
C. Csallner and Y. Smaragdakis, “JCrasher: an automatic robust-
ness tester for Java,” Software Practice and Experience, vol. 34, no. 11,
pp. 1025–1050, 2004.
[8]
C. Pacheco and M. D. Ernst, “Randoop: Feedback-directed
...
Programming
Systems
and Applications Companion, ser. OOPSLA ’07.
New York,
NY,
USA:
ACM,
...
[9]
C. Pacheco, S. K. Lahiri, and T. Ball, “Finding errors in .net
...
’08.
New York, NY, USA: ACM, 2008, pp. 87–96. [Online].
...
M. W. Hall, Eds.
ACM, 2005, pp. 213–223. [Online]. Available:
https://doi.org/10.1145/1065010.1065036
[13] K. Sen, D. Marinov, and G. Agha, “CUTE: a concolic unit
...
H. C. Gall, Eds.
ACM, 2005, pp. 263–272. [Online]. Available:
...
di Vimercati, Eds.
ACM, 2006, pp. 322–335. [Online]. Available:
...
and
P. Gr¨unbacher, Eds.
ACM, 2014, pp. 385–396. [Online]. Available:
...
A. Zeller, Eds.
ACM, 2011, pp. 416–419. [Online]. Available:
...
-
October 2, 2020.
IEEE, 2020, pp. 523–533. [Online]. Available:
...
Austin, Texas, USA, May 14-22, 2016.
ACM, 2016, pp. 5–14.
...
J. Lang, Ed.
ijcai.org, 2018, pp. 4159–4165. [Online]. Available:
...
22-30 May 2021.
IEEE, 2021, pp. 150–162. [Online]. Available:
...
2017, San Francisco, California, USA, S. Singh and S. Markovitch,
Eds.
...
D. Marinov, Eds.
ACM, 2014, pp. 437–440. [Online]. Available:
...
M. Papadakis and S. R. Vergilio, Eds., vol. 13711.
Springer,
...
M. Whalen, Eds.
IEEE Computer Society, 2015, pp. 190–200.
...
in ACM/IEEE International Conference on Automated Software
Engineering,
ASE
’14,
Vasteras,
